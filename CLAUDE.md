...
> **The deployed manifests ship the real rules, byte-identical to the controller's apply.** The rules and `matchConditions` are static, so `deploy/base` and the chart carry the same wildcard rule + CEL conditions the controller SSA-applies — both appliers converge on identical values and co-own the fields without churn. `rules` is an **atomic** list: the earlier `rules: []` "bootstrap" meant every reconciliation by a GitOps applier (kustomize-controller applies with Force) wiped the controller-applied rules, silently disarming the webhook until the next `--vwc-resync-interval` tick.

> **From-scratch recreation is fail-open, warned, and safe.** `buildWebhookEntry` applies the safety-critical fields — `failurePolicy: Ignore`, `timeoutSeconds: 3`, `matchPolicy: Equivalent` and the system-namespace `namespaceSelector` (`kube-system`/`kube-public`/`kube-node-lease` + the webhook's own namespace) — so a VWC deleted out-of-band and recreated by the controller never gets the apiserver defaults (`failurePolicy: Fail`, no selector), which would fail closed cluster-wide with no caBundle. The `cert-manager.io/inject-ca-from` annotation is deliberately **not** applied by the controller (the Certificate name is deployment-specific — the chart derives it from the release fullname, and cert-manager may be absent with a static `tls.caBundle`), so a recreated VWC serves no TLS until the manifest is reapplied: fail-open under the applied `Ignore`, surfaced by a `ConfigRecreated` **Warning** Event. The recreation path needs the `create` verb on validatingwebhookconfigurations in the ClusterRole — SSA of a missing object is authorized as a create — and only e2e T13 exercises that grant under real RBAC (unit tests use an RBAC-free fake client, envtest runs as admin). RBAC is otherwise least-privilege: VWC `update`/`patch` are scoped via `resourceNames` to the managed VWC (unscoped writes would let a compromised pod neutralise every other admission control), and leader-election leases live in a namespaced Role scoped to `flux-drift-webhook-leader`.
...
| `--failure-policy` | `Ignore` | VWC `failurePolicy` applied by the controller (`Ignore` or `Fail`); must match the deployed manifest or the two SSA appliers churn (env: `FAILURE_POLICY`) |
| `--webhook-timeout` | `3` | VWC `timeoutSeconds` applied by the controller (1–30); must match the deployed manifest |
| `--extra-excluded-namespaces` | *(empty)* | CSV of extra namespaces appended to the VWC namespaceSelector exclusions, after the built-ins, in order (env: `EXTRA_EXCLUDED_NAMESPACES`) |
| `--system-controller-sas` | *(empty)* | Extra control-plane identities (CSV of `namespace:name` SA shorthands or full `system:` usernames) allowed to CREATE Flux-labelled derived resources and DELETE Flux-applied resources (lifecycle); merged with built-in defaults. A namespace literally named `system` cannot be allow-listed via shorthand (reserved-prefix guard) (env: `SYSTEM_CONTROLLER_SAS`) |
...
- the safety-critical fields (`failurePolicy: Ignore`, `timeoutSeconds: 3`, `matchPolicy: Equivalent`, system-namespace `namespaceSelector`) are applied — not left to apiserver defaulting — and survive the round-trip on both entries
- from-scratch recreation after an out-of-band delete: fail-open defaults on the recreated object, `ConfigRecreated` Warning emitted once, absent on the following steady-state pass
- the production cache wiring (`webhook.CacheOptions`): a real informer serves namespace reads as `PartialObjectMetadata` with `managedFields` stripped by the transform pipeline
